...
export class AppComponent {
  ...
  // An array of previous calculations, for reference
  previousCalculations = new Array();
  ...
  computeAnswer() {
    ...
    // Save this calculation to the previous calculations
    this.addCalculationToHistory(this.operationText+"="+resultAsString);
    this.resultText = resultAsString;
    this.operationText = this.defaultOperationText;
  }

  lastHistoryEntryId = 0;

  /**
   * Adds a previous calculation to a div so the user can see what has been calculated before.
   * Entries will be clickable to restore the result and possibly entire calculation.
   * @param calculation the full calculation as a string to add to the calculation history
   */
  addCalculationToHistory(calculation:string){
    this.previousCalculations.push(calculation);
    var historyElement = document.createElement("div");
    historyElement.setAttribute("class","historyDivEntry");
    var tempId = "historyDivEntryNumber"+(this.lastHistoryEntryId++);
    historyElement.setAttribute("id",tempId);
    historyElement.addEventListener("click",this.restoreCalculation.bind(this),false);
    historyElement.textContent=calculation;
    document.getElementById("historyDiv").append(historyElement);
  }

  /**
   * Restores the result of the calculation stored in the div element that fired the click event.
   * @param event created by document
   */
  restoreCalculation(event){
    var elementId = event.target.id;
    var element=document.getElementById(elementId);
    var historyString = element.textContent;
    var result = historyString.split("=")[1];
  ...
  /**
   * Clears the calculation history by removing the text content from the history div.
   */
  clearHistory(){
    var historyDiv = document.getElementById('historyDiv');
    historyDiv.textContent="";
    this.previousCalculations =  new Array();
  }
